...
class brandModel extends Connection
{
    ...
    {
        $sql = "SELECT * FROM `$table` WHERE $table.`status` = 1 ORDER BY 'id' DESC  ";
    ...
    {
        $sql = "SELECT `$table`.`name` as `brand_name`,`$table_product`.* FROM `$table` INNER JOIN `$table_product` ON `$table`.`id` = `$table_product`.`brand_id` WHERE `$table`.`id` = $id AND $table_product.`status`= 1 ORDER BY `$table_product`.`id` DESC LIMIT $offset,$limit ";
        return $this->db->select($sql);
    }
    public function GetProductById($table, $table_product, $id)
    {
        $sql = "SELECT * FROM `$table` INNER JOIN `$table_product` ON `$table`.`id` = `$table_product`.`brand_id` WHERE `$table`.`id` = $id AND $table_product.`status`= 1 ORDER BY `$table_product`.`id` DESC";
        return $this->db->select($sql);
    }
    public function SelectOrder($table, $sort, $limit = 6, $offset = 0)
    {
        $sql = "SELECT * FROM `$table` ORDER BY $sort DESC LIMIT $offset,$ WHERE $table.`status` = 1";
        return $this->db->select($sql);
    }
