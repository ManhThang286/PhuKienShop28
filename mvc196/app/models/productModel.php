...
class productModel extends Connection
{
    ...
    {
        $sql = "SELECT * FROM $table WHERE id =:id AND $table.`status` = 1 ";
        $data = [
            ':id' => $idsanpham
        ];
    ...
    {
        $sql = "SELECT * FROM `$table` INNER JOIN `$table_product` ON `$table`.`id` = `$table_product`.`category_id` WHERE `$table_product`.`category_id` = $id  AND `$table_product`.`status` != 0 ORDER BY `$table_product`.`id` DESC";
        return $this->db->select($sql);
    }
    public function GetProductByIdPagination($table, $table_product, $id, $limit = 15, $offset = 0)
    {
        $sql = "SELECT `$table`.`name` as `category_name`,`$table_product`.* FROM `$table` INNER JOIN `$table_product` ON `$table`.`id` = `$table_product`.`category_id` WHERE `$table`.`id` = $id AND $table_product.`status` != 0 ORDER BY `$table_product`.`id` DESC LIMIT $offset,$limit";
        return $this->db->select($sql);
    }
    ...
    {
        $sql = "SELECT * FROM `$table` WHERE `$table`.`name` LIKE  '%$textSearch%' AND $table.`status` = 1";
        return $this->db->select($sql);
    }
    public function searchProductPagination($table, $textSearch, $limit = 12, $offset = 0)
    {
        $sql = "SELECT * FROM `$table` WHERE `$table`.`name` LIKE  '%$textSearch%' AND $table.`status` = 1 ORDER BY `id` DESC LIMIT $offset,$limit ";
        return $this->db->select($sql);
    }
